...
function build_javadocs() {
  ...
  local start=`date`
  cd ${PROJECT_PATH}
  MAVEN_OPTS="-Xmx4g -XX:MaxPermSize=256m" mvn clean install -P examples,templates,release -DskipTests -Dgpg.skip=true && mvn ${javadoc_run} -DskipTests -DisOffline=false ${debug_flag}
